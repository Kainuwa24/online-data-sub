...
/** In-memory / session flag so we don't re-prompt on every client navigation */
export const BIOMETRIC_SESSION_OK_KEY = "ods-biometric-session-ok";
...
export async function authenticateBiometric(
  ...
  try {
    ...
    return {
      verified: false,
      message: e instanceof Error ? e.message : "Biometric verification failed",
    };
  }
}

/** @deprecated Prefer authenticateBiometric() for cancel vs fail; kept for call sites expecting boolean */
export async function authenticateBiometricSimple(options?: AuthenticateOptions) {
  const result = await authenticateBiometric(options);
...
/** Human-readable reason when biometrics are unavailable (for Security settings). */
